...
namespace InvoiceAPI.Services
{
    public class InvoiceService : IInvoiceService
    {
        ...
        public InvoiceService(WelldoneContext dbContext, HtmlEncoder htmlEncoder, IConfiguration config)
        {
            ...
            List<invoice_product_record> invoiceProducts)
        {
            ...
            for (int attempt = 1; attempt <= maxRetries; attempt++)
            {
                ...
                {
                    // 產生可用發票號碼
                    var invoiceNumber = await CreateAvailableInvoiceNumber(systemCodeId);
                ...
                catch (DbUpdateConcurrencyException ex)
                {
                    ...
                    {
                        throw new BusinessException($"發票開立失敗，重試 {maxRetries} 次後仍未成功", ex);
                    }
                ...
                {
                    await transaction.RollbackAsync();
                    throw new BusinessException($"發票開立失敗: {ex.Message}", ex);
                }
            }
        ...
        private async Task<invoice_system_user_serial_map> UserBindingAsync(invoice_system_code systemCode, string userId)
        {
            ...
            for (int attempt = 1; attempt <= maxRetries; attempt++)
            {
                ...
                catch (DbUpdateConcurrencyException ex)
                {
                    ...
                    if (attempt == maxRetries)
                    {
                        throw new BusinessException($"使用者綁定失敗，重試 {maxRetries} 次後仍未成功", ex);
                    }
                    // 隨機延遲以避免重試衝突
                    await Task.Delay(delayMs + new Random().Next(0, 300));
                }
            ...
            throw new BusinessException("使用者綁定失敗，請稍後重試");
        }
        ...
        /// <returns></returns>
        /// <exception cref="BusinessException"></exception>
        private async Task<string> CreateAvailableInvoiceNumber(int systemCodeId)
        {
        ...
        private invoice_record GetNewInvoiceRecord(IssueReq req, int userMapId, string userCarrierId)
        {
            ...
            return new invoice_record
            {
                ...
                buyer_name = RandomUtil.GetCommonRandomNumber(4),
